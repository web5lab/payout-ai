...
describe("Escrow Contract (Unit)", function () {
    ...
    describe("ERC20 Deposits", function () {
        it("Should accept ERC20 token deposits", async function () {
            ...
            await expect(escrow.connect(offeringContract).depositToken(paymentToken.target, depositAmount))
                .to.emit(escrow, "Deposited")
                .withArgs(offeringContract.address, paymentToken.target, depositAmount);
            const deposit = await escrow.deposits(offeringContract.address);
            expect(deposit.amount).to.equal(depositAmount);
        ...
        it("Should revert ERC20 deposit if token address is zero", async function () {
            const { escrow, offeringContract } = await loadFixture(deployEscrowFixture);
            const depositAmount = ethers.parseUnits("100", 18);
            await expect(escrow.connect(offeringContract).depositToken(ethers.ZeroAddress, depositAmount)).to.be.revertedWith("Invalid token");
        });

        it("Should revert ERC20 deposit if amount is zero", async function () {
            const { escrow, offeringContract, paymentToken } = await loadFixture(deployEscrowFixture);
            await expect(escrow.connect(offeringContract).depositToken(paymentToken.target, 0)).to.be.revertedWith("Invalid amount");
        });
    ...
    describe("Refunds", function () {
        it("Should allow owner to refund native deposit to investor", async function () {
            ...
            const depositAmount = ethers.parseEther("1.0");
            await escrow.connect(investor).depositNative(offeringContract.address, { value: depositAmount });
        ...
        it("Should allow owner to refund ERC20 deposit to investor", async function () {
            ...
            await paymentToken.mint(offeringContract.address, depositAmount);
            await paymentToken.connect(offeringContract).approve(escrow.target, depositAmount);
            await escrow.connect(offeringContract).depositToken(paymentToken.target, depositAmount); // Here, offeringContract is the investor for this deposit
            await escrow.connect(owner).enableRefunds();
            await expect(escrow.connect(owner).refund(offeringContract.address, offeringContract.address)) // Refund to offeringContract as investor
                .to.emit(escrow, "Refunded")
                .withArgs(offeringContract.address, offeringContract.address, paymentToken.target, depositAmount)
                .and.to.changeTokenBalances(
                    paymentToken,
                    [escrow, offeringContract],
                    [-depositAmount, depositAmount]
                );
    ...
    describe("Withdrawals", function () {
        it("Should allow owner to withdraw native ETH", async function () {
            ...
            const depositAmount = ethers.parseEther("1.0");
            await escrow.connect(investor).depositNative({ value: depositAmount });
        ...
        it("Should allow owner to withdraw ERC20 tokens", async function () {
            ...
            await paymentToken.mint(offeringContract.address, depositAmount);
            await paymentToken.connect(offeringContract).approve(escrow.target, depositAmount);
            await escrow.connect(offeringContract).depositToken(paymentToken.target, depositAmount);
